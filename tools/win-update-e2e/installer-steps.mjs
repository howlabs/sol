...
//
// Sol ships a per-user oneClick NSIS installer (electron-builder defaults:
// oneClick=true, perMachine=false) named sol-windows-setup.exe. One-click
// silent mode is `<setup.exe> /S`; the app installs under
// %LOCALAPPDATA%\Programs\<dir> and the exe is Sol.exe. The install dir casing
...
export function silentInstall(setupExe, { timeoutMs = 180_000, installDir = null } = {}) {
  ...
  // On update runs the old Sol.exe already exists, so wait for the exe whose
...
/**
 * Locate the installed Sol.exe. In isolated mode (`installDir` set), the exe is
 * at a known fixed path (<installDir>\Sol.exe). Otherwise it is discovered
 ...
 * developer's REAL Sol. It additionally refuses to run against the default
  ...
  if (!allowDefaultLocation && pathsEqual(resolved, path.join(programsRoot(), PRODUCT_NAME))) {
    throw new Error(
      `Refusing to uninstall the default install location "${resolved}" — this is where a ` +
        `developer's REAL Sol lives. Isolated mode must target a separate --install-dir.`
    )
  }
