...
/**
 * Read-only discovery of the machine's current Sol install registry state.
...
export function discoverInstallRegistryState() {
  const ps = [
    ...
    `  if ($dn -eq 'Sol' -or ($il -and $il -match '\\\\Programs\\\\orca')) {`,
    ...
    `  if ($il -and ($sn -eq 'Sol' -or $il -match 'orca')) {`,
...
/** The two directories NSIS writes Sol shortcuts into: Start Menu + Desktop. */
...
/** Read-only: list Sol *.lnk shortcuts under the given dirs (recursive). */
...
/**
 * Snapshot the shared install registry keys + Sol shortcuts before an isolated
 ...
 * Pre-existing shortcuts are copied back; test-created Sol shortcuts are removed.
...
function printMismatchWarning(manifest, expected, actual) {
  ...
  console.error('!! Your REAL Sol install pointer may be hijacked to the test directory.')
  console.error('!! The next real Sol update could install into the test location.')
...
if (process.argv[1] && path.resolve(process.argv[1]) === import.meta.filename) {
  ...
  console.log('[registry-shortcut-backup] Sol shortcuts that would be backed up:')
