...
Installer source (version N, then N+1) — path or release tag on each side:
  --from <path>            Local sol-windows-setup.exe for the base version (N)
  --to <path>              Local sol-windows-setup.exe for the update (N+1)
...
Options:
  --install-dir <path>     Isolated-install mode: install the test build into
                           ...
                           leaving a developer's REAL Sol install untouched.
  ...
  --allow-existing-install Proceed even if a Sol install already exists. The
                           ...
                           developer's real Sol. Clean machines (CI/VM) never
...
/** Default per-user oneClick install location: %LOCALAPPDATA%\Programs\Sol. */
function defaultInstallDir() {
  const localAppData =
    process.env.LOCALAPPDATA ?? path.join(process.env.USERPROFILE ?? '', 'AppData', 'Local')
  return path.join(localAppData, 'Programs', 'Sol')
}
...
function looksLikeHarnessInstall(dir) {
  return existsSync(path.join(dir, 'Sol.exe')) && existsSync(path.join(dir, 'Uninstall Sol.exe'))
}
    ...
    if (entries.length > 0 && !looksLikeHarnessInstall(installDir)) {
      errors.push(
        `--install-dir "${installDir}" is a non-empty directory that does not look like a ` +
          `prior harness install (no Sol.exe + "Uninstall Sol.exe"). Refusing to overwrite ` +
