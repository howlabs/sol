...
// watches our exit code). Each scenario mimics a real Sol usage pattern:
//
      ...
      sub = await watcher.subscribe(
        ...
        (err, events) => {
          if (err) {
            recordError(err)
            errored = true
            // Sol's createWatcher error path: unsubscribe from inside the
            // error callback.
            if (sub) {
              stats.unsubscribes++
...
async function main() {
  ...
  // Outside the repo worktree: a live Sol instance may be watching the repo,
