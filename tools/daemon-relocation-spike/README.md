...
> `win-unpacked` build so that a **copied `Sol.exe`** — run with
...
The daemon is `fork()`ed from the app's own Electron binary (`Sol.exe`) with
...
round-trips the nonce, the daemon's main module is the copied `Sol.exe`, and
**no** loaded module resolves under `--app-dir`.

## Tiers (defined as data in `tier-file-set.mjs`)

Every tier includes the irreducible core: `Sol.exe`, `icudtl.dat`, both V8
...
process's **main module** is the copied `Sol.exe`, not the install-dir one.
...
- Whether `TIER_MINIMAL` (no top-level DLLs) boots `Sol.exe` as node at all, or
...
ship minimal; if `Sol.exe` fails to boot without the non-GPU DLLs, fall back to
