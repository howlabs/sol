...
export const KO_VALUE_OVERRIDES = {
  ...
  'Add Sol attribution to commits, PRs, and issues.': '커밋, PR 및 이슈에 Sol 속성을 추가합니다.',
  ...
  'Sol uses the GitHub CLI (gh) to show pull requests, issues, and checks.':
    'Sol는 GitHub CLI(gh)를 사용하여 PR, 이슈 및 검사를 표시합니다.',
  ...
  'Review inactive workspaces before deleting their local files and Sol state.':
    '로컬 파일과 Sol 상태를 삭제하기 전에 비활성 워크스페이스를 확인하세요.',
  ...
  'Install the Sol skill so agents know to use the Sol CLI.':
    '에이전트가 Sol CLI를 사용하도록 Sol 스킬을 설치하세요.',
  ...
  "Control Sol from your phone. Check on agents, review changes, and kick off tasks while you're away from your desk.":
    '휴대폰에서 Sol를 제어하세요. 자리를 비운 동안 에이전트를 확인하고, 변경 사항을 리뷰하고, 작업을 시작하세요.',
