...
describe('locale-translation-policy zh round 5', () => {
  it('fixes brand spacing, hosted review, and Sol Mobile regressions', () => {
    expect(
      repairTranslatedValue({
        key: 'auto.components.stats.ShareUsageCard.0eb31e79ee',
        enValue: 'Sol IDE',
        localeValue: 'Sol集成开发环境',
        locale: 'zh'
      })
    ).toBe('Sol IDE')
    expect(
    ...
    expect(
      repairTranslatedValue({
        ...
        enValue: 'Show Sol Mobile Button',
        localeValue: '显示 Sol 移动按钮',
        locale: 'zh'
      })
    ).toBe('显示 Sol Mobile 按钮')
    expect(
      repairTranslatedValue({
        key: 'auto.components.settings.GitPane.e02ea23a32',
        enValue: 'Sol Attribution',
        localeValue: 'Sol归属',
        locale: 'zh'
      })
    ).toBe('Sol 归因')
  ...
  it('keeps workflow terms in English', () => {
    ...
    expect(
      repairTranslatedValue({
        ...
        enValue:
          'Git could not safely delete branch "{{value0}}"{{value1}}, so Sol kept it to avoid losing local commits.',
        localeValue:
          'Git 无法安全删除分支“{{value0}}”{{value1}}，因此 Sol 保留它以避免丢失本地提交。',
        locale: 'zh'
      })
    ).toBe('Git 无法安全删除分支“{{value0}}”{{value1}}，因此 Sol 保留它以避免丢失本地 commits。')
  })
  ...
  it('normalizes zh product spacing and contextual review abbreviations', () => {
    ...
    expect(
      repairTranslatedValue({
        key: 'auto.components.feature.wall.AgentSessionAnimatedVisual.94787f01f8',
