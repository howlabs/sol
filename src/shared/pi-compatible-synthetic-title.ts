...
// Why: legacy Pi shells can emit the delimiter before cwd text exists.
...
export function getPiCompatibleSyntheticAgentLabel(
  ...
  if (!PI_COMPATIBLE_SYNTHETIC_TITLE_RE.test(title)) {
    return null
  }
  return 'Pi'
}
...
export function getPiCompatibleSyntheticAgentStatus(
  ...
  // Why: bare "Pi" and ready/idle/done labels are all idle. Bare labels come
  // from normalizeTerminalTitle collapsing π frames; they must re-detect as
  // idle or stored lastOscTitle values classify as neutral after main-side
  // normalization.
  return 'idle'
}
