...
// the relay sends to Sol. Consumed by `src/relay/agent-hook-server.ts` (which
...
// between Sol's main process and the remote relay.
//
// Per the design doc:
// - The relay normalizes; Sol routes. The envelope's `payload` field has
//   already been through `normalizeHookPayload` (which calls
//   `parseAgentStatusPayload` → `normalizeAgentStatusObject`) on the relay
//   side. Sol's `ingestRemote` re-runs the canonical payload normalizer at
...
// - The wire `connectionId` is **always `null`**: a `connectionId` is Sol's
//   local handle on an `ssh2` connection, not a wire identity. Sol stamps the
...
//   POST body so Sol's warn-once protocol diagnostics still fire. The relay
...
// with the same value so Sol can attribute the event back to the right CLI.
...
/** Wire envelope for a single hook event flowing relay → Sol. */
export type AgentHookRelayEnvelope = {
  ...
  tabId?: string
  worktreeId?: string
  /** Always `null` on the wire — relay does not know Sol's local connectionId. */
  connectionId: null
  /** Preserved from the relay-side normalized hook event so Sol can
  ...
  /** True when the relay is replaying its cache after Sol reconnects. */
  isReplay?: boolean
  ...
  env?: string
  /** Forwarded verbatim from the agent CLI POST body. Lets Sol's warn-once
   ...
   *  Sol's `ingestRemote` validates it again at the SSH trust boundary. */
...
/** JSON-RPC request method Sol issues after `--connect` reattach to ask the
...
/** JSON-RPC request method Sol issues at session-ready to ship the
...
/** Feature-flag env var. Read once at process start by Sol and the relay.
