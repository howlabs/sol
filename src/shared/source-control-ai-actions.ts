...
export const SOURCE_CONTROL_ACTION_VARIABLE_INFO: Record<string, SourceControlActionVariableInfo> =
  {
    basePrompt: {
      description:
        'Sol’s built-in prompt for this action, including the context Sol knows how to gather safely.',
      example:
    ...
    firstPrompt: {
      description: 'The first user request that created the Sol workspace.',
      example: 'Fix CI and commit the result'
    },
    assistantMessage: {
      description: 'The initial agent response, when Sol has one available.',
