...
async function performAddWorktree(
  ...
  // every Sol-created worktree wants the same default. True per-worktree
  ...
  //   with the "benign and idempotent" rationale above — every Sol-created
 ...
 * for this op. The caller owns migrating Sol's path-derived worktree identity
...
async function performRemoveWorktree(
  ...
  try {
    // Why: `git worktree remove` only detaches the filesystem entry. Sol also
