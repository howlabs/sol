...
import {
  DESKTOP_RELEASE_ATOM_FEED_URL,
  desktopReleaseTagHrefRegex,
...
export function getReleaseDownloadUrl(tag: string): string {
  return getDesktopReleaseDownloadUrl(tag)
}
...
async function fetchReleaseFeedTags(): Promise<ReleaseFeedTag[] | null> {
  try {
    const res = await net.fetch(DESKTOP_RELEASE_ATOM_FEED_URL, {
    ...
    for (const match of body.matchAll(desktopReleaseTagHrefRegex())) {
      const tag = match[1]
      const version = normalizeTagToVersion(tag)
