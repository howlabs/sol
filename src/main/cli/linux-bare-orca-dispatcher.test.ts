...
async function makeFixture(): Promise<{ homePath: string; resourcesPath: string }> {
  ...
  // The bundled sol launcher must exist for the dispatcher to be written.
...
describe('installLinuxBareOrcaDispatcher', () => {
  it('writes an executable bare-orca dispatcher that execs the bundled sol launcher', async () => {
    ...
    const expectedTarget = join(resourcesPath, 'bin', 'sol')
  ...
  it('quotes a resources path containing spaces so the exec line cannot be split', async () => {
    ...
    const content = await readFile(result.dispatcherPath, 'utf8')
    expect(content).toContain(`exec '${join(resourcesPath, 'bin', 'sol')}' "$@"`)
  })
  ...
  it('skips when the bundled sol launcher is missing from the build', async () => {
