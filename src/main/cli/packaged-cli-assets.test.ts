...
const linuxLauncherAsset = new URL('../../../resources/linux/bin/sol', import.meta.url)
  ...
  itRunsUnixShell(
    ...
    async () => {
      ...
      try {
        ...
        const launcherPath = join(launcherDir, 'sol')
        const electronPath = join(appDir, 'sol')
...
`,
        ...
        const commandPath = join(commandDir, 'sol')
  ...
  itRunsUnixShell('runs the AppImage CLI wrapper through APPDIR at runtime', async () => {
    ...
    try {
      ...
      const commandPath = join(root, 'sol')
      await mkdir(cliDir, { recursive: true })
      await writeFile(
        cliPath,
        `exports.main = (argv) => {
