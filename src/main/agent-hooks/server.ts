...
// Why: this module is the Sol-main-process adapter for the shared
...
//     Sol restart so retained dashboard rows reappear on relaunch
...
// Why: name of the on-disk cache that survives Sol restart. Lives next to
...
export class AgentHookServer {
  ...
  private port = 0
  private token = ''
  // Why: identifies this Sol instance so hook scripts can stamp requests and
   ...
   *  identity Sol holds (the wire envelope carries connectionId: null and
   *  Sol stamps the real value here). The relay has already normalized the
  ...
  stop(): void {
    ...
    // would introduce a TOCTOU race vs. a concurrent Sol instance.
