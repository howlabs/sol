...
// so Sol manages only its own marker-delimited block: install rewrites the
...
// Why: mirror the Claude-compatible events Sol normalizes for status. Kimi uses
...
const BLOCK_START = '# >>> orca-managed-kimi-hooks (managed by Sol; do not edit) >>>'
...
// Sol generates; escape both defensively anyway.
...
// Sol-managed script (by filename, so a moved userData path is still swept).
