 ...
 * image is the install-dir Sol.exe and its loaded modules (node-pty native,
 ...
 * of Sol.exe (renamed to a distinct image name) is byte-identical, so
...
// The relocated host exe is a copy of Sol.exe renamed to a distinct image
// name. The NSIS updater's name-based kill (`taskkill /IM Sol.exe`) matches by
...
const DAEMON_HOST_EXE_NAME = 'sol-terminal-daemon.exe'

// V8 snapshots + ICU data the Electron bootstrap reads even under
// ELECTRON_RUN_AS_NODE; siblings of Sol.exe in win-unpacked.
...
export function buildDaemonHostManifest(sources: DaemonHostSources): CopyOp[] {
  ...
  // `taskkill /IM Sol.exe` can't match it; the blobs beside it are read by the
