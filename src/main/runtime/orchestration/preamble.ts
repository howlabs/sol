export type PreambleParams = {
  ...
  // shells have no agent prompt for Sol to reuse.
...
// Why: the dispatch preamble teaches agents about Sol's CLI commands for
...
export function buildDispatchPreamble(params: PreambleParams): string {
  ...
  // production CLI and talk to the wrong Sol instance (Section 6.4).
  ...
  const header = `You are working inside Sol, a multi-agent IDE. You are a dispatched worker.
...
prompt for Sol to reuse; if the coordinator has more for you it will
