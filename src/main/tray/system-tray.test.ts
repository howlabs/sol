...
describe('createSystemTray', () => {
  it('creates a tray with a Sol tooltip and Open/Quit menu on win32', async () => {
    setPlatform('win32')
    ...
    expect(trayInstances[0].setToolTip).toHaveBeenCalledWith('Sol')
    const items = builtMenuItems()
    expect(items.map((i) => i.label)).toEqual(['Open Sol', undefined, 'Quit'])
    expect(items[1].type).toBe('separator')
  })

  it('wires Open Sol, the tray click, and Quit to their callbacks', async () => {
    setPlatform('win32')
    const { createSystemTray } = await loadModule()
    const onOpen = vi.fn()
    const onQuit = vi.fn()
    ...
    const items = builtMenuItems()
    items.find((i) => i.label === 'Open Sol')?.click?.()
