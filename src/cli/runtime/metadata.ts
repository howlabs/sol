  ...
  try {
    ...
    if (!metadata || !findTransport(metadata, 'unix', 'named-pipe') || !metadata.authToken) {
      throw new RuntimeClientError(
        'runtime_unavailable',
        `Sol runtime metadata is incomplete at ${metadataPath}`
      )
    }
    return metadata
  } catch (error) {
    ...
    throw new RuntimeClientError(
      'runtime_unavailable',
      `Could not read Sol runtime metadata at ${metadataPath}. Start the Sol app first.`
    )
  }
}
...
export function getDefaultUserDataPath(
  ...
  // Why: in dev mode (and for parallel Sol instances), the Electron app writes
  ...
  if (platform === 'darwin') {
    return join(homeDir, 'Library', 'Application Support', 'sol')
  }
  if (platform === 'win32') {
    ...
    if (!appData) {
      throw new RuntimeClientError(
        'runtime_unavailable',
        'APPDATA is not set, so the Sol runtime metadata path cannot be resolved.'
      )
    }
    return join(appData, 'sol')
  }
  ...
  return join(process.env.XDG_CONFIG_HOME || join(homeDir, '.config'), 'sol')
}
