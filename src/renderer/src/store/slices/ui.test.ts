...
function makeLinearIssue(overrides: Partial<LinearIssue> = {}): LinearIssue {
  return {
    ...
    labels: [],
    labelIds: [],
    team: { id: 'team-1', name: 'Sol', key: 'ORC' },
...
function makeJiraIssue(overrides: Partial<JiraIssue> = {}): JiraIssue {
  return {
    ...
    project: { id: '10000', key: 'ORC', name: 'Sol', siteId: 'site-1' },
...
describe('createUISlice feature tips', () => {
  it('marks feature tips seen and persists them once', () => {
    ...
    store.getState().markFeatureTipsSeen(['orca-cli'])
    store.getState().markFeatureTipsSeen(['orca-cli'])

    expect(store.getState().featureTipsSeenIds).toEqual(['orca-cli'])
    expect(setMock).toHaveBeenCalledTimes(1)
    expect(setMock).toHaveBeenCalledWith({ featureTipsSeenIds: ['orca-cli'] })
  })
  ...
  it('normalizes persisted feature tip ids during hydration', () => {
    ...
    store.getState().hydratePersistedUI(
      makePersistedUI({
        featureTipsSeenIds: ['orca-cli', 'unknown', 'orca-cli'] as never
      })
    )
