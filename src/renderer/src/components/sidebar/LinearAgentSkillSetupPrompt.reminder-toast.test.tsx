...
function cliStatus(overrides: Partial<CliInstallStatus>): CliInstallStatus {
  return {
    ...
    pathConfigured: true,
    launcherPath: '/Applications/Sol.app/Contents/MacOS/Sol',
    installMethod: 'symlink',
    supported: true,
    state: 'installed',
    currentTarget: '/Applications/Sol.app/Contents/MacOS/Sol',
...
describe('LinearAgentSkillSetupPrompt reminder toast', () => {
  ...
  it('shows a warning toast on a later modal-only activation after Not now', async () => {
    ...
    expect(toast.warning).toHaveBeenCalledWith(
      'Sol CLI and Linear skill are missing',
      expect.objectContaining({
        id: 'linear-agent-skill-setup-orca.linearTicketsSkill.setupDismissed.host',
        description:
          'Install the Sol CLI and the Linear skill to enable your agents to read and edit Linear tasks.',
        action: {
          label: 'Set up',
  ...
  it('does not repeat the Sol CLI in CLI-only reminder toast copy', async () => {
    ...
    expect(toast.warning).toHaveBeenCalledWith(
      'Sol CLI is missing',
      expect.objectContaining({
        description: 'Install the Sol CLI to enable your agents to read and edit Linear tasks.'
      })
    )
  })
  ...
  it('keeps remote setup nuance in reminder toast copy', async () => {
    ...
    expect(toast.warning).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        description:
          'Install the Sol CLI and the Linear skill to enable your agents to read and edit Linear tasks. Remote agent environments may need their own setup.'
      })
    )
  })
  ...
  it('keeps WSL target nuance in reminder toast copy', async () => {
    ...
    expect(toast.warning).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        description:
          'Install the Sol CLI and the Linear skill to enable your agents to read and edit Linear tasks. This setup runs in the selected WSL agent runtime.'
      })
    )
  })
