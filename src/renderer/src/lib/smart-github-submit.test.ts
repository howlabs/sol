...
describe('getSmartGitHubSubmitIntent', () => {
  it('treats GitHub issue and pull URLs as submit-time source intent', () => {
    expect(getSmartGitHubSubmitIntent('https://github.com/howlabs/sol/pull/2049')).toEqual({
      kind: 'link',
      owner: 'stablyai',
      repo: 'orca',
      number: 2049,
    ...
    expect(getSmartGitHubSubmitIntent('https://github.com/howlabs/sol/issues/2050')).toEqual({
      kind: 'link',
      owner: 'stablyai',
      repo: 'orca',
      number: 2050,
  ...
  it('finds an embedded GitHub item URL when prose punctuation touches the URL', () => {
    expect(
      getSmartGitHubSubmitIntent('review (https://github.com/howlabs/sol/pull/2049), please')
    ).toEqual({
      kind: 'link',
      owner: 'stablyai',
      repo: 'orca',
    ...
    expect(getSmartGitHubSubmitIntent('fix https://github.com/howlabs/sol/issues/2050.')).toEqual({
      kind: 'link',
      owner: 'stablyai',
      repo: 'orca',
      number: 2050,
...
describe('lookupSmartGitHubSubmitItem', () => {
  ...
  it('reuses an in-flight direct URL lookup for the same repo and intent', async () => {
    const item = {
      ...
      state: 'open' as const,
      url: 'https://github.com/howlabs/sol/pull/2049',
      labels: [],
  ...
  it('prunes expired distinct lookup entries on later lookups', async () => {
    ...
    const workItem = vi.fn().mockImplementation(({ number }) =>
      Promise.resolve({
        ...
        title: `Issue ${number}`,
        state: 'open' as const,
        url: `https://github.com/howlabs/sol/issues/${number}`,
        labels: [],
        updatedAt: '2026-05-26T00:00:00.000Z',
        author: 'octocat',
        repoId: 'repo-1'
...
describe('getSmartGitHubSubmitResolution', () => {
  ...
  it('strips duplicated issue prefixes while preserving linked issue metadata', () => {
    const resolution = getSmartGitHubSubmitResolution({
      ...
      url: 'https://github.com/howlabs/sol/issues/2050'
    })
  ...
  it('uses the URL path to normalize stale PR-typed issue results', () => {
    expect(
      getSmartGitHubSubmitResolution({
        ...
        url: 'https://github.com/howlabs/sol/issues/6933'
      })
    ).toEqual({
