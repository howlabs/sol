...
describe('ai vault resume command runtime', () => {
  ...
  it('queues a PowerShell-valid local Pi resume by session id', () => {
    const state = makeState({ worktreePath: 'C:\\Users\\alice\\repo' })
    ...
    const command = buildAiVaultResumeCommandForWorktree({
      ...
      session: {
        ...
        filePath: 'C:\\Users\\alice\\.pi\\agent\\sessions\\repo\\sess.jsonl',
        cwd: 'C:\\Users\\alice\\repo',
        codexHome: null
      }
    })
    ...
    expect(command).toBe(
      "Set-Location -LiteralPath 'C:\\Users\\alice\\repo'; pi --session '019f27cd-4268-7000-96e7-62f42a55c144'"
    )
  })
