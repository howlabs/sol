...
export const AGENT_SKILL_CLI_PREREQUISITE_NOTICE =
  'Before opening setup, Sol may show a system prompt to register the Sol CLI command on PATH.'

export const CLI_PREREQUISITE_REGISTRATION_TOAST = 'Sol needs to register its CLI on PATH.'
export const CLI_PREREQUISITE_REGISTRATION_TOAST_DESCRIPTION =
  'Approve the system prompt so skill setup can use the Sol CLI command.'
    ...
    toast.error(
      error instanceof Error
        ...
        : translate(
            ...
            'Failed to register the Sol CLI in PATH.'
          )
    )
    return null
  }
...
function showCliPrerequisiteWarning(status: CliInstallStatus): void {
  if (!status.supported) {
    toast.warning(
      translate(
        ...
        'Sol CLI registration is unavailable'
      ),
      {
        description:
          status.detail ??
          translate(
            ...
            'Install the Sol CLI before running agent skill setup.'
          )
      }
    )
    return
  ...
  if (status.state !== 'installed') {
    toast.warning(
      translate(
        ...
        'Sol CLI registration needs attention'
      ),
      {
        description:
          status.detail ??
  ...
  if (!status.pathConfigured) {
    ...
    // Sol affordances when that shell can resolve the Sol CLI command.
    toast.warning(
      translate(
        'auto.lib.agent.skill.cli.prerequisite.79371593b0',
        'Sol CLI is not visible on PATH yet'
      ),
      {
        description:
          status.detail ??
          translate(
            ...
            'Restart your shell or add the Sol CLI directory to PATH before setup.'
          )
      }
    )
  }
