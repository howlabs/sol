...
REM launcher resolves APP_DIR back to resources/ and `sol open` cannot find
REM Sol.exe on packaged Windows installs.
for %%I in ("%RESOURCES_DIR%\..") do set "APP_DIR=%%~fI"
set "ELECTRON=%APP_DIR%\Sol.exe"

if not exist "%ELECTRON%" (
  echo Unable to locate Sol.exe next to "%RESOURCES_DIR%" 1>&2
  exit /b 1
)

REM Why: Sol packages the CLI entrypoint outside app.asar so the public shell
REM command can execute it directly with ELECTRON_RUN_AS_NODE.
