#!/bin/bash
# Why: register the bundled `sol` CLI on PATH at package-install time.
# The in-app "Install CLI" action (CliInstaller) can never run on a headless
# server, so without this symlink `sol serve` is unreachable from the shell on
...
for dir in /opt/Sol /opt/sol; do
  ...
  shim="$dir/resources/bin/sol"
  if [ -x "$shim" ]; then
    # Only manage our own symlink; never clobber an unrelated /usr/bin/sol.
